...
class ControlPanel(QWidget):
    def __init__(self, model, update_callback):
        ...
        self.dynamic_start_time = None  # Track when dynamic mode is activated
        self.init_ui()
    ...
    def toggle_power(self, checked):
        ...
        if not checked:
            self.dynamic_start_time = None  # Reset dynamic simulation on power off
        self.update_callback()
    ...
    def toggle_dynamic_mode(self, checked):
        ...
        if checked and self.model.power_on:
            self.dynamic_start_time = time.time()  # Start dynamic simulation only if power is on
        else:
            self.dynamic_start_time = None  # Reset if turning off or power is off
        self.update_callback()
...
class MainWindow(QMainWindow):
    ...
    def update_plots(self):
        ...
        # Apply dynamic parameters only if dynamic mode is active and started
        ...
        # Simulate input current (lagging without PFC)
        ...
        # Update harmonic analyzer for THD+N
        self.harmonic_analyzer.update_plots()
        ...
        # Update analysis display with synchronized metrics
        try:
            ...
            ripple = avg_voltage = phase = power = thd_n = 0.0
            temperature = 25.00
            efficiency = 100.00
            power_factor = 1.00
